		...
		if (index == -1)
		{
			...
			int chosen;
				...
				cout << "1- Add\n"
					<< "2- Reduce\n"
					<< "0- Exit\n";
				chosen = ut.getint(0, 2, "> ");
				switch (chosen) {
				case 0:
					cout << "Aborted!\n" << endl;
					break;
				case 1: {
					int checkFull = Product[index]->qtyNeeded() - Product[index]->qty();
					if (checkFull == 0) {
						...
						while (true) {
							if (cin >> addQty && addQty >= 1 && addQty <= checkFull) {
								break;
							}
							else {
								cout << "Value out of range [" << 1 << "<=val<=" << checkFull << "]: ";
								cin.clear();
								cin.ignore(numeric_limits<streamsize>::max(), '\n');
							}
						}
						*Product[index] += addQty;
				...
				case 2: {
					int checkFull = Product[index]->qty();
					if (checkFull != 0) {
						int reduceQty;
						cout << "Quantity to reduce: ";
						while (!(cin >> reduceQty) || reduceQty < 1 || reduceQty > checkFull) {
							cout << "Value out of range [" << 1 << "<=val<=" << checkFull << "]: ";
							cin.clear();
							cin.ignore(numeric_limits<streamsize>::max(), '\n');
						}
						*Product[index] -= reduceQty;
